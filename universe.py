...
def check_password():
    ...
    if "password_correct" not in st.session_state:
        st.title("Omniverse Observation Station")
...
# --- 1. Main Setup ---
st.set_page_config(layout="wide", page_title="Omniverse Simulator Pro v4.9")
...
def angle_ui(label, key_base, def_val):
    ...
    st.slider(f"{label} Slider", 0, 360, key=f"{key_base}_s", step=1, on_change=update_from_slider, args=(key_base,), label_visibility="collapsed")
    st.number_input(f"{label} Box", 0.0, 360.0, key=f"{key_base}_b", step=0.01, format="%.2f", on_change=update_from_box, args=(key_base,), label_visibility="collapsed")
...
with tab2:
    ...
    with st.expander("Surface Skin", expanded=True):
        ...
        ui_sf_d, ui_sf_p, ui_sf_sync = "Turquoise", "#00A2E8", False
        ui_sf_op = 0.3
        if ui_show_surf:
            ...
            if op_b != st.session_state.op_val: st.session_state.op_val = op_b
            elif op_s != st.session_state.op_val: st.session_state.op_val = op_s
...
def apply_changes():
    ...
    st.session_state['draw_params'] = {
        ...
        'segments_main': int(ui_seg_m), 'segments_sub': int(ui_seg_s), 'smoothness': int(ui_smooth),
        'bg_color': bg, 'show_bb': ui_show_bb, 'bb_shape': ui_bb_shape, 'bb_size': ui_bb_size,
        'bb_color': resolve_color(ui_bb_d, ui_bb_p, False, bg),
        'show_surf': ui_show_surf, 'surf_color': resolve_color(ui_sf_d, ui_sf_p, ui_sf_sync, bg), 'surf_opacity': ui_sf_op,
        'outer_color': resolve_color(ui_out_d, ui_out_p, ui_out_sync, bg), 'lw_outer': ui_lw_out,
        'inner_color': resolve_color(ui_in_d, ui_in_p, ui_in_sync, bg), 'lw_inner': ui_lw_in,
        'wire_start': ui_w_st, 'wire_span': ui_w_sp, 'surf_start': ui_s_st, 'surf_span': ui_s_sp,
        'camera_mode': ui_cam, 'rot_x': int(ui_rx), 'rot_y': int(ui_ry), 'rot_z': int(ui_rz)
    }
...
try:
    ...
    if P['show_surf']:
        ...
        fig.add_trace(go.Surface(x=rxs.reshape(xs.shape), y=rys.reshape(ys.shape), z=rzs.reshape(zs.shape), surfacecolor=np.ones_like(xs), colorscale=[[0, P['surf_color']], [1, P['surf_color']]], showscale=False, opacity=P['surf_opacity'], contours=dict(x=dict(highlight=False), y=dict(highlight=False), z=dict(highlight=False)), hoverinfo='none', lighting=dict(ambient=0.7, diffuse=0.8, roughness=0.5)))
    ...
    if P['show_bb']:
        if P['bb_shape'] == "circle":
            ...
            xb, yb, zb = r_bb * np.sin(vb) * np.cos(ub), r_bb * np.sin(vb) * np.sin(ub), r_bb * np.cos(vb)
            fig.add_trace(go.Surface(x=xb, y=yb, z=zb, surfacecolor=np.ones_like(xb), colorscale=[[0, P['bb_color']], [1, P['bb_color']]], showscale=False, opacity=1.0, hoverinfo='none'))
        else:
            fig.add_trace(go.Scatter3d(x=[0], y=[0], z=[0], mode='markers', marker=dict(size=P['bb_size'], color=P['bb_color'], symbol=P['bb_shape'], opacity=1.0), name="Big Bang"))
    ...
    fig.update_layout(scene=dict(xaxis=dict(visible=False), yaxis=dict(visible=False), zaxis=dict(visible=False), bgcolor=P['bg_color'], aspectmode='data'), paper_bgcolor=P['bg_color'], margin=dict(l=0, r=0, b=0, t=0), scene_camera=dict(eye=eye, center=center), height=800)
